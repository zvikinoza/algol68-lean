...
/-- Whether an assigning operator on a name of mode `m1` with a right operand of mode `m2`
    is one of those.  A slot written only by assignment and by these still qualifies for
    promotion to a C variable, so `storeAssignOp` must be able to emit every case this
    admits — the two are read together. -/
def assignOpOk (op : String) (m1 m2 : Mode) : Bool :=
  match m1 with
  | .ref im =>
    match CTy.ofMode im with
    | some ty => CTy.ofMode m2 == some ty && (assignOpExpr op ty "x" "y").isSome
    | none => false
  | _ => false
...
partial def slotEscapesV (d sl : Nat) (c : Core) : Bool :=
  ...
  -- an assigning operator on a name of primitive mode is a read and a write of the slot,
  -- no more than a plain assignment is
  | .dyop op m1 m2 dst src =>
    if assignOpOk op m1 m2 then
      (match strip dst with
       | .refCell dd ss => if dd == d && ss == sl then false else slotEscapes d sl dst
       | dst' => slotEscapes d sl dst') || slotEscapes d sl src
    else slotEscapes d sl dst || slotEscapes d sl src
...
partial def genVoid (c : Core) : M Unit := do
  ...
  | .dyop op m1 m2 lhs rhs =>
    if (← storeAssignOp op m1 m2 lhs rhs) then pure ()
    else if op == "+:=" && (← appendTo m1 lhs rhs) then pure ()
    else do gen c; emit "a68_v(a68rt_pop(W));"
  | _ => gen c; emit "a68_v(a68rt_pop(W));"

/-- `x +:= e` on a name of primitive mode, written straight into its C variable or its
    cell.  Whenever `assignOpOk` holds and the destination is a whole cell this must
    apply, because a slot written this way may have been promoted to a C variable and so
    have no cell for the general path to reach. -/
partial def storeAssignOp (op : String) (m1 m2 : Mode) (dst src : Core) : M Bool := do
  if !assignOpOk op m1 m2 then return false
  match m1, strip dst with
  | .ref im, .refCell dd ss =>
    match CTy.ofMode im with
    | none => return false
    | some ty =>
      let promoted ← lvar dd ss
      -- everything that can fail is settled before anything is emitted
      match promoted with
      | some (_, vt, _) => if vt != ty then return false
      | none => pure ()
      -- the right operand natively, or left on the operand stack and taken off again;
      -- `assignOpExpr` uses it exactly once, so the one pop happens exactly once
      let rhs ← match scalarExpr (← env) ty.toMode src with
        | some e => pure e
        | none => do gen src; pure s!"{ty.popFn}()"
      match promoted with
      | some (v, vt, u) =>
        let e := (assignOpExpr op vt (readVar (v, vt, u)) rhs).getD rhs
        emit (s!"{v} = {e};" ++ (if u then s!" {v}_i = 1;" else ""))
        return true
      | none =>
        let d ← rtd dd
        let e := (assignOpExpr op ty s!"{ty.cellFn}({d}, {ss})" rhs).getD rhs
        emit s!"{ty.setFn}({d}, {ss}, {e});"
        return true
  | _, _ => return false
...
static inline uint32_t a68_repr(int64_t x) {
  if (x < 0 || x > 255) { a68_v(a68rt_arith_error(5, W)); return 0; }
  return (uint32_t) x;
}
"
