...
partial def substLitStmt (slot : Nat) (v : Value) (d : Nat) : CoreStmt → CoreStmt
  | .decl s init => .decl s (substLit slot v d init)
      ...
      | .decl slot init =>
        ...
        | .lit v =>
          ...
          let declared := out.foldl (fun n st => match st with
            | .decl s _ => if s == slot then n + 1 else n
            | _ => n) 0
          let named := out.any fun
            | .decl _ c => refsSlot slot 0 c
...
def cseStmt (e0 : Core) (t : Nat) (s : CoreStmt) (st : CseSt) : CoreStmt × CseSt :=
  ...
  match s with
  | .decl slot init => let (i', st) := cseGo e0 t init st; (.decl slot i', st)
  ...
  Id.run do
    ...
    | some (k, p) =>
      ...
      for j in [0:out.size] do
        if j == k then res := res.push (.decl t init)
        res := res.push out[j]!
...
partial def cseBlock (fuel : Nat) (fsize : Nat) (stmts : Array CoreStmt) (nl : Nat) :
  ...
  if nl != 0 || fuel == 0 then (fsize, stmts) else Id.run do
    ...
    for s in stmts do
      match s with
      | .decl _ c | .unit c => cands := collectShareable c cands
      | _ => pure ()
...
partial def optStmt (rt : Interp.Rt) (cfg : Cfg) : CoreStmt → IO CoreStmt
  | .decl slot init => return .decl slot (← opt rt cfg init)
